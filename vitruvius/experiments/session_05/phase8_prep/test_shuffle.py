...
def test_content_multiset_preserved():
    ...
    for b in range(ids.shape[0]):
        ...
        content_orig = sorted(i for i, a in zip(ids_row, attn[b].tolist())
                              if a == 1 and i not in SPECIAL)
        content_sh = sorted(i for i, a in zip(sh_row, attn[b].tolist())
                            if a == 1 and i not in SPECIAL)
...
def test_special_positions_unchanged():
    ...
    shuffled = shuffle_input_ids(ids, attn, SPECIAL, seed=1729)
    for b in range(ids.shape[0]):
        for i, (orig, new) in enumerate(zip(ids[b].tolist(), shuffled[b].tolist())):
            if orig in SPECIAL:
